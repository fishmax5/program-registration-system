...
function refreshFormForNewDates(formId, group, configInfo) {
  ...
  // applyProgramFormExtensions() below (99r).
...
function buildFormTitleForGroup(group) {
  ...
  // as long as the program runs (see 94). A form still called "Low-Cost Wills
...
function writeEventRegistryRows(registrySheet, group, formInfo) {
  ...
  const rows = collapseSessionsByEventId_(group).map(session => {
    ...
    // What the top of the form says — see 99r. Kept current afterwards by
