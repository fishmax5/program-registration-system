...
//
// doGet() serves four different pages off ONE deployment address, told apart
// by ?mode= alone, and the ways that goes wrong are all silent: a tablet on
...
//      has been through a QR generator and back collects parameters.
...
sandbox.buildWalkInHtml = opts => `PAGE:walkin roster=${opts.rosterUrl}`;
...
const ROUTED = [
  ...
  [{ mode: 'walkin' }, 'walkin', '?mode=walkin'],
  [{ mode: 'walk-in' }, 'walkin', '?mode=walk-in'],
  [{ mode: 'legacy' }, 'walkin', '?mode=legacy'],
  [{ mode: 'WALK-IN' }, 'walkin', '?mode=WALK-IN'],
  [{ view: 'walkin' }, 'walkin', '?view=walkin'],
...
ok('the walk-in page is handed a roster link built from the table',
  served({ mode: 'walkin' }).html ===
    'PAGE:walkin roster=https://script.google.com/macros/s/ABC/exec?mode=session');
...
ok('mode: walkin', sandbox.checkInPageUrl({ mode: 'walkin' }) === `${BASE}?mode=walkin`);
...
ok('mode: walk-in likewise',
  sandbox.checkInPageUrl({ mode: 'walk-in' }) === `${BASE}?mode=walkin`);
