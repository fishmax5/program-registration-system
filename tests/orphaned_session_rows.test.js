...
// no longer reads (Code.gs section 7b).
//
...
// --- The blank Calendar_Source that keeps triage off the meals (section 5b) ---
{
  const rows = [
...
// --- What the confirmation prompt actually reads ---
{
  ...
  const many = [];
  for (let i = 0; i < 9; i++) many.push(row(`x${i}`, `Program ${i}`, new Date(2026, 7, i + 1), RETIRED));
  ...
  ok('reports the row count', /9 row\(s\)/.test(text), text);
