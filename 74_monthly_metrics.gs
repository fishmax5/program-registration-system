// ============================================================================
// 74 — MONTHLY METRICS, AND THE YEAR-OVER-YEAR SUMMARY BUILT ON THEM
// ============================================================================
//
...
//      deleted deliberately. Any comparison computed live from Registrant_Dash
...
function collectMetricsSourceRows(ss) {
  ...
  return {
    sessionRows: sessionSheet
      ? readAllSectionedRowValues(sessionSheet, HEADERS.Master_Program_Dashboard, 'Event_ID') : [],
    sessionMap: getIndexMap(HEADERS.Master_Program_Dashboard),
    registrantRows: registrantSheet
      ? readAllSectionedRowValues(registrantSheet, HEADERS.Registrant_Dash, 'Event_ID') : [],
    registrantMap: getIndexMap(HEADERS.Registrant_Dash)
  };
}
 ...
 * carries: "first seen" means "has no earlier row on Registrant_Dash". Once a
...
function writeMetricsSheet(sheet, rows, now) {
  ...
  writeSectionBanner(sheet, 1, numCols, '📈 Metrics — Year Over Year', {
    ...
    note: 'One row per month, counted once and then kept. The rows a month was counted from are eventually archived; ' +
      ...
      'Recount from Reports → "Update Metrics Now". The Notes column is yours; a recount carries it.'
  });
