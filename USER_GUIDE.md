...
| **🔍 Check Event Links vs the Dashboard (read-only)** | Reads every upcoming calendar event and compares the registration link in its description against the form the dashboard names for that same session, then reports every program where the two disagree. Changes nothing. A disagreement means residents reading the calendar and staff reading the dashboard are being sent to two different sign-up pages, with the registrations splitting between them — invisible from either side alone. **🔗 Rewrite Event Links** (above) is the fix; check first that the dashboard names the form you want kept, because that is the one everybody will be sent to |
| **🗑️ Recover Deleted Forms…** | For when somebody deletes forms out of the Drive folder. Checks every form this workbook depends on — the ones the dashboard links to, the ones the next sync would reuse, and the lunch sign-up forms — and says which are fine, which are sitting in the Drive trash, which are alive but no longer in the folder, and which Drive cannot produce at all. It reports before it writes anything. Restoring from the trash **keeps every link and every response already collected**; forms outside the folder are simply filed back. Only forms that cannot be recovered at all lead to a rebuild, and only after a second, separate prompt that spells out that those get new links |
