...
/**
 * The ?mode= spellings that ask for the PREVIOUS door page rather than the
 * app. Several of them on purpose, for the same reason CHECK_IN_ROSTER_MODES
 * (section 16b) has several: these URLs are typed by hand onto tablets and
 * read off printed cards, and "walk-in" opening the wrong page because the
 * code spells it "walkin" is a fault nobody at a door can diagnose.
 */
const CHECK_IN_WALK_IN_MODES = ['walkin', 'walk-in', 'legacy'];
...
defineLazyGlobal_('DOOR_ROUTES', () => [
  ...
  },
  {
    ...
    mode: 'walkin',
    title: 'Sign In',
    match: params => CHECK_IN_WALK_IN_MODES.indexOf(doorRequestedMode_(params)) !== -1,
    build: (params, ctx) => buildWalkInHtml({
      location: ctx.location,
      pinRequired: ctx.pinRequired,
      locations: ctx.locations,
      rosterUrl: checkInPageUrl({ location: ctx.location, mode: 'session' })
    })
  },
  {
    id: 'door',
