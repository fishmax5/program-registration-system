...
defineLazyGlobal_('DOOR_ROUTES', () => [
  ...
  },
  {
    ...
    //
    // ?span= IS THE ONE THING ITS URL CARRIES BESIDES THE MODE, and it is why
    // there can be two printed links: a weekly one for a newsletter that goes
    // out on Mondays and a monthly one for a flyer, opening on the same page
    // showing different amounts of it. It only chooses which filter is
    // already pressed — every filter is still there and still one tap away,
    // so neither link is a page that hides anything from whoever opens it.
    match: params => PUBLIC_CALENDAR_MODES.indexOf(doorRequestedMode_(params)) !== -1,
    build: params => buildPublicCalendarHtml(publicProgramCalendar({}),
      { span: publicCalendarSpanRequested_(params) })
  },
  {
    id: 'session',
    mode: 'session',
