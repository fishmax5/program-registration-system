...
// her back?") is the one thing that copy makes dangerous.
//
// `officeCopy: true` sends the same office addresses their own message instead:
// the member's copy names the member alone, and a second message goes to the
// office list a moment later, subject prefixed and headed with who received
// the original. Office staff reply-all on that one freely; it cannot reach the
// member, because they were never on it.
//
// It costs exactly what the BCC cost — one message per office address — so a
// caller may switch between the two without touching its reserve. THE PRIMARY
// SEND IS WHAT COUNTS: a forward that fails is logged and nothing more, and
// never turns a delivered message into a 'failed' one, because the person was
// told.
//
...
function sendRationedEmail(request) {
  ...
  const forwardToOffice = !!req.officeCopy && bccList.length > 0;
  if (bccList.length > 0 && !forwardToOffice) options.bcc = bccList.join(',');
  ...
  if (forwardToOffice) sendOfficeCopy_(options, bccList, req.describeRecipient);
...
/**
 * The office's own copy of a message that has just gone to somebody outside
 * the organization. See THE OFFICE COPY in this file's banner for why this is
 * a separate message and not a BCC line.
 *
 * NEVER THROWS, and its cost is already accounted for by the caller: the
 * message it copies is away, the quota was checked against both, and a failed
 * forward must not turn a delivered reminder into a failure the next sync
 * sends again.
 */
function sendOfficeCopy_(sentOptions, officeAddresses, describeRecipient) {
  try {
    MailApp.sendEmail({
      to: officeAddresses.join(','),
      subject: `${OFFICE_COPY_SUBJECT_PREFIX}${sentOptions.subject}`,
      body: `${describeRecipient || `Sent to ${sentOptions.to}`}.\n` +
        `Replies to this copy stay in the office — ${sentOptions.to} is not on it.\n\n` +
        `${'-'.repeat(60)}\n\n${sentOptions.body}`
    });
  } catch (err) {
    log(`\u26a0\ufe0f Could not send the office copy of "${sentOptions.subject}" ` +
      `to ${officeAddresses.join(', ')} (${err}).`);
  }
}
