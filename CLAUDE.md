...
| `07_dates_and_labels.gs` | 685 | `TIMEZONE`, date formatting, and the capacity/meal hints a form's date label carries. |
...
| `13_lunch_only_signup_form.gs` | 1062 | The lunch-only sign-up form — the one form built from `Lunch_Schedule` rather than the calendar. Also the shared sheet-shaping helpers it grew around: `autosizeColumns`, and `ensureSheetColumns` (a layout that outgrows a tab's column count is a throw, not a resize). |
...
| File | | What is in it |
|---|--:|---|
| `43_program_dashboard.gs` | 1055 | `renderProgramDashboard`. Still the home of the metrics block's arithmetic and every one of its column notes (`computeProgramMetrics` / `writeProgramMetricsSection`) — the block itself is now DRAWN on `Master_Program_Dashboard` (`78`), the tab whose grain it matches. |
