...
| `73_door_app_html.gs` | 1312 | `buildDoorAppHtml` — the whole served app, one template literal, five screens redrawn into one `<main>`. **The events screen comes first and is why the name list is readable**: the desk ticks what THIS tablet is for (everything on at that building that day, whether or not anybody has registered — the count is shown instead of the event being hidden), everything ticked to begin with, and every screen after it is about those events. **The name list then has two sections** — registered for the ticked events, then `Here recently`, the people who came to those same programs in the last two months and have not registered for today (`DAY.past`, from `74`), tapped into the same personal screen with the desk's ticks already on. A weekly class has the same eight people in it every week and half of them have never filled in a form. The membership application screen went with the endpoints behind it; the question is still asked on the walk-in screen, because it is the only place anybody ever asks it. |
