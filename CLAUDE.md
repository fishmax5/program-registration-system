...
| File | | What is in it |
|---|--:|---|
| `60_check_in_page_server.gs` | 1192 | `doGet` and `DOOR_ROUTES` — the ordered table of every page this one deployment serves that WRITES to the workbook (cancel first, the door app last and matching everything, including a retired `?mode=walkin`/`walk-in`/`legacy` bookmark), which `checkInPageUrl` also builds its `?mode=` from so a link and the router cannot drift. The two PUBLIC pages are no longer in this table — `doGet` asks `publicEmbedRoute` (`88`) before it walks this one, and `doorRouteUrlMode_` asks `publicEmbedUrlMode` before it answers, because the one thing those pages need (being frameable) is a property of the RESPONSE that a row here has no way to say. Plus `checkInRosterModeRequested`, the PIN gate, the roster read, and the mark/register handlers. |
...
and (with `90`) one of the two that another website may frame. Numbered last
for the usual reason: both are behavior only, their own constants stand alone,
and everything they reach for (`HEADERS`, `getSectionedRows`,
`deskMonthHorizonKey`, the tag readers, `tryGetScriptCache`) they read at CALL
time or through a hoisted function declaration.

| File | | What is in it |
|---|--:|---|
| `86_public_program_calendar.gs` | 286 | **What is on, and the form to sign up with — for a stranger.** `publicProgramCalendar()` is the ungated read behind `?mode=public`: every session from today to the end of NEXT month (`deskMonthHorizonKey`, the same horizon the door's month picker uses), each carrying the CURRENT registration link for that session — which for a Regular program is the thing an emailed link is always wrong about a month later. It is safe to publish because of what `buildPublicSessionRow()` does NOT put in it: **no names, no counts of who, no `All_Registrants` read at all** — only what is already printed on a calendar event a member can see, plus whether there is room (`publicSeatsPhrase_`, exact at the bottom end and vague at the top: "3 seats left" changes an afternoon, "37 registered" is an internal number). Rows that must not be shown are dropped SERVER-side (past, beyond the horizon, untitled, `Cancelled`) — a filter in a public page's JavaScript is not a filter — and `publicSessionState_` resolves the four things a stranger can be told: sign up, join the waiting list, just come along (`No_Registration`), or not yet (the sessions are on the calendar but no form is generated, which is ordinary a month out). The link is read out of the `=HYPERLINK()` FORMULA in `Form_Response_Link`, because the cell's value is the words "View Live Form". Cached in CacheService for `PUBLIC_CALENDAR_CACHE_SECONDS` under a key with today's date in it — a TTL alone would go on offering yesterday as the first day past midnight. **No PIN, deliberately**: it writes nothing, and a page that asks a stranger for a staff code is a page that becomes a phone call. Its session field list is PINNED by `tests/public_calendar.test.js`, which is why `89` derives the weekday it needs from `dayLabel` rather than adding a field. |
| `87_public_program_calendar_html.gs` | 350 | `buildPublicCalendarHtml` — the served page, one template literal, drawn on the shared embed stylesheet (`publicEmbedStyles`, `88`). **The whole two-month window is inlined**, so the first frame is the answer and the week / month / everything filters, the building filter and the search are arithmetic in the browser rather than a round trip apiece; the only server call is the quiet re-read after the first paint (and the Refresh control), which never redraws under a finger. The building is a row of PILLS and an outlined chip on every card — it was a `<select>` and flat gray text, which on the website's colored block hid the second question every caller asks — and the controls sit in a white card because a transparent control bar on a transparent page was a row nobody could see. A tap marks its own card as opening in the same frame and the form opens in its own tab, so the calendar a person spent four taps filtering is still behind it. Everything from the workbook is written with `textContent` — the page uses no `innerHTML` with data in it at all — and the snapshot crosses into script through the same double-`JSON.stringify` every other page here uses. `tests/public_calendar.test.js` pins the field list that may leave the workbook, the four states and the routing; `tests/inline_pages_parse.test.js` compiles its script block. |

### The public embeds and the weekly page (88–90)

The two pages that live on somebody ELSE'S website, and the one property that
separates them from everything else this deployment serves: they may be
FRAMED. Numbered last for the usual reason — never renumber — and safely so:
all three are behavior only, `PUBLIC_EMBED_ROUTES` is lazy (`01a`), the mode
lists and the two weekly constants stand alone, and every page they name is a
hoisted function.

| File | | What is in it |
|---|--:|---|
| `88_public_embeds.gs` | 263 | **The public router, and the one place `ALLOWALL` is written.** `PUBLIC_EMBED_ROUTES` declares the two public pages (`PUBLIC_CALENDAR_MODES`, moved here from `60`, and `PUBLIC_WEEKLY_MODES`), `publicEmbedRoute` claims a request, `publicEmbedUrlMode` is what keeps a printed `?mode=` and this table together, and `servePublicEmbed` serves them with `XFrameOptionsMode.ALLOWALL` — which is why they left `60`: every page in `DOOR_ROUTES` WRITES to the workbook, so letting any site frame one is how a tap on somebody else's page becomes a check-in on this one, and the door's serving path must never say ALLOWALL. Serving the calendar the ordinary way is also why the embed on the website was a blank block: the browser refused the frame. Also `publicEmbedStyles()`, the stylesheet BOTH public pages are drawn on — transparent body (the website's own section color is the background), white self-painting surfaces, black pill buttons, and no `prefers-color-scheme` override, because the block behind it is one fixed color whatever the phone is set to. |
| `89_weekly_programs.gs` | 194 | **What runs every Tuesday, decided from the dates.** Nothing in this workbook stores "this is a weekly class" — the recurrence lives in the calendar — so `foldWeeklyPrograms_` reads it off the sessions: same weekday, gaps a whole number of weeks, smallest gap exactly one (the rule `detectProgramMonthRecurrence` in `78` uses, and for the same reason — a weekly class that misses a week has gaps [1, 2, 1]), at least `PUBLIC_WEEKLY_MIN_SESSIONS` of them. Every OTHER week is deliberately not on the page. The key is **title + building**, because a `[Shared]` Tuesday program is two things a person can go to at two addresses, and the entry describes the NEXT session — its link, its seats, its time. Built by folding what `publicProgramCalendar()` already returned: one tab read, one cache, two pages, and no second promise about what a stranger may see. |
| `90_weekly_programs_html.gs` | 266 | `buildPublicWeeklyHtml` — the served page, one template literal on `publicEmbedStyles()`. Seven headings (**Every Monday** … **Every Sunday**), one card per PROGRAM rather than per date, the dates themselves a quiet line under the chips ("Next: Thursday, October 2 · 8 more through November 27") because the promise this page makes is the weekday. Same building pills, same search, same inlined-fold-then-quiet-re-read as `87`, same `textContent`-only rule. `tests/weekly_programs.test.js` pins what counts as weekly, the field list, the routing and the framing; `tests/inline_pages_parse.test.js` compiles its script block. |
